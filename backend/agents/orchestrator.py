"""
orchestrator.py
Master agent with x402, ERC-8004, AIsa — works with Groq or Anthropic.
"""
...
class Orchestrator:
    ...
    def plan_task(self, task: str) -> list:
        ...
        prompt = f"""Break this task into discrete micro-actions.
...
Respond ONLY with a JSON array. Max 14 items."""
    ...
    def run(self, task: str, specialists: dict) -> dict:
        ...
        for step in plan:
            ...
            self.emit("agent_done", {"agent": agent_id, "action": action, "result": (result or "")[:120]})
            time.sleep(0.3)
